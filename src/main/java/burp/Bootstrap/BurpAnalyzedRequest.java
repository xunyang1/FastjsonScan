...
public class BurpAnalyzedRequest {
    ...
    public byte[] getNewRequest(String payload, List<String> newHeaders){
        byte[] newRequest;
    ...
    public IHttpRequestResponse makeHttpRequest(String payload, List<String> newHeaders) {
        byte[] newRequest = getNewRequest(payload, newHeaders);
    ...
    /**
     * 会根据程序类型自动组装请求的 请求发送接口并计算响应时间
     */
    public long makeHttpRequest_Delay(String payload, List<String> newHeaders){
        byte[] newRequest = getNewRequest(payload, newHeaders);

        long l = System.currentTimeMillis();
        this.newRequestResponse = this.callbacks.makeHttpRequest(this.requestResponse().getHttpService(), newRequest);
        long delay = System.currentTimeMillis() - l;
