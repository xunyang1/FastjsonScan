...
import burp.Application.DelayExtension.Delay;
import burp.Ui.Tags;
...
public class BurpExtender implements IBurpExtender, IScannerCheck, IExtensionStateListener {
    ...
    public List<IScanIssue> doPassiveScan(IHttpRequestResponse baseRequestResponse) {
        ...
        // 基础请求分析
        // 检测请求中是否存在json参数，并将合格的参数放入eligibleJsonParameters，等待检测
        ...
        try {
            ...
            //延迟探测扩展
            IScanIssue delayIssuesDetail = this.delayExtension(tagId, baseAnalyzedRequest);
            if (delayIssuesDetail != null) {
                issues.add(delayIssuesDetail);
                return issues;
            }
    ...
    private IScanIssue delayExtension(int tagId, BurpAnalyzedRequest analyzedRequest) throws ClassNotFoundException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        String provider = this.yamlReader.getString("application.DelayExtension.config.provider");

        if (!this.tags.getBaseSettingTagClass().isStartdelayExtension()) {
            return null;
        }

        Delay delay = new Delay(this.globalVariableReader, this.callbacks, analyzedRequest, this.yamlReader, provider);
        if (!delay.run().isIssue()) {
            return null;
        }

        IHttpRequestResponse httpRequestResponse = delay.run().getHttpRequestResponse();

        this.tags.getScanQueueTagClass().save(
                tagId,
                delay.run().getExtensionName(),
                this.helpers.analyzeRequest(httpRequestResponse).getMethod(),
                new CustomBurpUrl(this.callbacks, httpRequestResponse).getHttpRequestUrl().toString(),
                this.helpers.analyzeResponse(httpRequestResponse.getResponse()).getStatusCode() + "",
                "[+] found fastJson command execution",
                delay.run().getHttpRequestResponse()
        );

        delay.run().consoleExport();
        return delay.run().export();
    }
